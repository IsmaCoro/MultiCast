...
app.config['MAX_CONTENT_LENGTH'] = 16 * 1024 * 1024 # Confuguracion para el peso de la imagen
...
def api_obtener_usuarios():
    ...
    for row in cursor.fetchall():
        usuario_dict = dict(zip(columns, row))
        # Esto coloca la visualización del link de la imagen
        if usuario_dict['ruta_foto']:
            usuario_dict['foto_url'] = f"http://localhost:5000/fotos/{usuario_dict['ruta_foto']}"
        usuarios.append(usuario_dict)
...
def api_agregar_usuario_web():
    try:
        ...
        if not all([nombre, apellidos, codigo_estudiante, curp, calle]):
        ...
        ruta_foto = f"user_{codigo_estudiante}{file_extension}"
     ...
     # Verificar si el código ya existe
        ...
        try:
            file.save(destino_imagen)
            mensaje_imagen = f"✅ Imagen guardada: {destino_imagen}"
        except Exception as e:
            mensaje_imagen = f"⚠️ Error guardando imagen: {e}"
